...
Your settings are now created by running the `dactyl-generate-configuration` command, which can be manipulated and shared.
...
Running the `dactyl-keyboard` command will automatically load a default configuration.
Running `dactyl-keyboard --config <config file>` will load and use the designated config file.
...
Before you begin, ensure you have installed [Anaconda](https://docs.anaconda.com/anaconda/install/index.html) and the `conda` command is available from your terminal.

You need to either add the relevant entry to your `environment.yml` file, as so:
```yaml
name: ...
channels:
dependencies:
   ...
   - pip:
     - "git+https://github.com/joshreve/dactyl-keyboard"
```

... or simply pip-install the same reference, as in the python environment instructions.

### Python Environment Installation

Before you begin, ensure you have installed [Python 3.X](https://www.python.org/downloads/release/python-3108).  It is advisable to setup a virtual environment to prevent package/library incompatibility.

Install via pip: `pip install git+https://github.com/joshreve/dactyl-keyboard`


## Generating the design

**Generating the design - UPDATED**
* (optional) Run `dactyl-generate-configuration` to generate a config file that can be manipulated to adjust the design.
* Run `dactyl-keyboard` to create the geometry
* This will generate a `things/` directory (or in subdirectory if defined in config)
    * `*left.*`
    * `*right.*`
    * `*plate.*`
    * `*oled_clip.*` (if applicable)
    * `*oled_clip_test.*` (if applicable)
* Use [CadQuery](https://github.com/CadQuery/cadquery) or other application to open a `.step` file
* Make changes to design, repeat run step
* When done, use CadQuery to export STL files
